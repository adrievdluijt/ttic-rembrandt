...
Do not force these terms into every observation — they would lose meaning. Use them where they actually do work. Specifically: do NOT invoke living-experience framing on organisational overview, educational, or marketing content where the reader is plausibly composed. The aim is that the framework should be legible in the prose where it earns its place, not pasted onto every issue.
...
3. Trust and grounding: does the content tell the reader they are not in trouble for reading it; is what happens next predictable; are options stated clearly; is difficulty acknowledged; are conditions hidden. For organisational and fundraising content: are trust claims evidenced on the page or do they point away to documents the reader is asked to fetch?
...
5. Reading age: estimate Flesch-Kincaid grade-level equivalent. Reading age is a proxy, not a target. Calibrate the target by audience:
   ...
   Do not present a flat "aim for grade 8" target regardless of audience. State the relevant target with brief context where it matters.

6. UK English surface check (regardless of selected jurisdiction): flag US spellings in content that is otherwise UK-coded (organize, organise; specialize, specialise; programs / programmes; behavior, behaviour; -ize / -ise endings; "math" vs "maths"). This is a separate, surface-level catch — list these under a brief note rather than as substantive issues. If the content is clearly US-targeted or the jurisdiction is US, do not flag US spellings.

7. Jurisdictional review for ${jurisdiction}: ${JURISDICTIONS[jurisdiction].frameworks}. Flag plausible concerns under these frameworks. Do NOT claim definitive compliance or non-compliance. Be specific about what would be flagged and why, never use vague "may not comply" phrasing.
   ...
   - ISO 22458 applies to services and consumer-facing content where vulnerability is a genuine operational consideration (financial services, utilities, telecoms, regulated services). Do NOT flag ISO 22458 on charity organisational overview pages, marketing content, blog posts, or educational content where the reader is not engaging the institution as a consumer at decision points. The standard is about institutional treatment of consumers in vulnerable circumstances, not about all writing that mentions difficulty.
   - GDS content standards apply to UK government digital content.
...
- If the rewrite needs an element the source does not provide (e.g. a specific impact figure to replace an aspirational claim), use an explicit bracketed placeholder with guidance for the writer — "[Insert specific figure here — e.g. percentage of income spent directly on programmes]" — rather than inventing the detail or leaving the gap unmarked.

...
- Do NOT recommend adding completion-time estimates ("takes about X minutes", "this should only take a moment", "it only takes a few minutes") to reassure the reader about a process. A time estimate is calibrated to a reader at full capacity; for a reader who is struggling, slowed, frightened or distracted, it becomes a standard they fail against and a source of self-blame when the task takes them longer. Where the source gives no realistic, reliable duration, do not invent one and do not prompt the writer to add one. Reduce the unknown a different way: remove time pressure and guarantee a route through if the reader gets stuck — but only using options, routes and facts the source already contains (see the no-new-propositions rule below). If the source offers no such route, raise the missing route as an issue rather than writing it in.
- Do NOT endorse or suggest unfalsifiable institutional reassurance — sentences whose negation no institution would ever write, and which therefore carry no information and simply ask the reader for trust. "We'll do everything we can", "we're committed to getting this right", "your experience matters to us", "we take this seriously" are credit requests, not facts. For an audience with reason to distrust the institution — people who have been failed, misled or made to wait by authority — trust-on-request is precisely the currency they are right to refuse, and the sentence reminds them of past betrayal rather than reassuring them. Flag these and push towards a verifiable statement the reader can check instead: a concrete fact, a named step, an option the reader controls, the absence of a specific pressure. This is distinct from the apologetic-preamble rule above: a boast is not an apology, it is a request for credit, and for a betrayed audience it lands worse. Do NOT suggest replacing one boast with a warmer boast.
- Do NOT endorse or suggest content in which the institution narrates the reader's own emotional state or sets the reader's pace. "This is a significant step", "we know this is difficult", "this must be hard for you" presume to name feelings the reader has not disclosed; "there's no rush", "take your time", "no need to hurry", "that's fine too" presume to grant permission or set a tempo for a decision that is the reader's to make. For a long-failed or long-delayed audience this is overreach twice over: the institution that caused the wait or the harm is the last party entitled to tell the reader how to feel about it or how quickly to move through it, and "that's fine" granting permission places the institution above the reader at the exact moment the content should be handing agency back. The trauma-informed move is not to perform awareness of the reader's state but to state plainly what is true and available and let the reader supply their own feeling and their own tempo: name the option, name the route to act on it, and stop. Flag instances where the institution assigns the reader an emotion, grants the reader permission, or dictates the reader's pace, and push towards stating the verifiable option instead.

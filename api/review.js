...
- Do NOT recommend adding completion-time estimates ("takes about X minutes", "this should only take a moment") to reassure the reader about a process. A time estimate is calibrated to a reader at full capacity; for a reader who is struggling, it becomes a standard they fail against and a source of self-blame when the task takes longer. Where the source gives no realistic, reliable duration, do not invent one and do not prompt the writer to. Reduce the unknown instead by removing time pressure ("there's no time limit", "you can stop and come back") and guaranteeing a human fallback ("if you get stuck, call me and I'll do it with you") — but only using routes and facts the source already contains.
